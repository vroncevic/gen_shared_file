...
Info
    Defines class Gen_shared_file with attribute(s) and method(s).
...
class GenSharedFile(ATSCli):
    '''
        Defines class GenSharedFile with attribute(s) and method(s).
        Loads a base info, create a CLI interface and run operation(s).
        ...
        It defines:
            ...
            :methods:
                | __init__ - Initials GenSharedFile constructor.
                | process - Processes and runs tool operation.
    '''

    _GEN_VERBOSE: str = 'GEN_SHARED_FILE'
    _CONFIG: str = '/conf/gen_shared_file.cfg'
    _LOG: str = '/log/gen_shared_file.log'
    _LOGO: str = '/conf/gen_shared_file.logo'
    _OPS: List[str] = ['-n', '--name', '-v', '--verbose']

    def __init__(self, verbose: bool = False) -> None:
        '''
            Initials GenSharedFile constructor.
        ...
        gen_shared_file_property: Dict[str, str | bool] = {
        ...
        Splash(gen_shared_file_property, verbose)
        ...
        if self.is_operational():
            self.add_new_option(
                ...
                help='generate shared file skeleton (provide name)'
            )
            self.add_new_option(
        ...
        if self.is_operational():
            try:
                ...
                if exists(f'{getcwd()}/{str(getattr(args, "name"))}'):
                    ...
                    return status
                gen: GenSharedFileSetup = GenSharedFileSetup(
