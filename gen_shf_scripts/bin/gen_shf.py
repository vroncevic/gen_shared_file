...
class GenSHF(CfgBase):
    """
        Define class GenSHF with attribute(s) and method(s).
    ...
    def __init__(self, verbose=False):
        """
            ...
            :exceptions: None
        """
        verbose_message(GenSHF.VERBOSE, verbose, 'Initial configuration')
        current_dir = Path(__file__).resolve().parent
        base_config_file = "{0}{1}".format(current_dir, GenSHF.__CONFIG)
        CfgBase.__init__(self, base_config_file, verbose=verbose)
        if self.tool_status:
            self.add_new_option(
                GenSHF.__OPS[0], GenSHF.__OPS[1], dest="pro",
                help="generate SHF project skeleton"
            )
    ...
    def process(self, verbose=False):
        ...
        if self.tool_status:
            ...
            if num_of_args == 1 and opts.pro and not project_exists:
                generator, gen_status = SHFSetup(verbose=verbose), False
